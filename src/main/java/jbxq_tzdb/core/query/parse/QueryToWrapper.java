package jbxq_tzdb.core.query.parse;
...
import jbxq_tzdb.core.query.data.Condition;
import jbxq_tzdb.core.query.data.Condition.Filter;
import jbxq_tzdb.core.query.data.Condition.Operator;
...
public class QueryToWrapper<T> {
	...
	public void parseSort(Wrapper<T> wrapper, Queryable queryable) {
		jbxq_tzdb.core.query.data.Sort sort = queryable.getPageable().getSort();
		if (sort != null) {
			for (jbxq_tzdb.core.query.data.Sort.Order order : sort) {
				if (order.getDirection() == jbxq_tzdb.core.query.data.Sort.Direction.DESC) {
					wrapper.orderBy(order.getProperty(), false);
				} else if (order.getDirection() == jbxq_tzdb.core.query.data.Sort.Direction.ASC) {
